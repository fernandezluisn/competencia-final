...
PARAM  <- list()
PARAM$experimento <- "TP_experimento"

#PARAM$exp_input  <- "base_3meses"
...
dataset_input  <- paste0( "./exp/paso2 drifting/datasetFINAL.csv.gz" )
...
if( PARAM$Tendencias ){
  TendenciaYmuchomas( dataset, 
                      ...
                      ventana=   5,      # 6 meses de historia
                      tendencia= TRUE,
                      minimo=    TRUE,
                      maximo=    TRUE,
                      promedio=  TRUE,
                      ratioavg=  TRUE,
                      ratiomax=  TRUE  )->dataset
}else{
  print("No corre tendencias")
}
...
#grabo el dataset
fwrite( dataset,
        "~/buckets/b1/exp/competencia final/dataset_5m.csv.gz",
        logical01= TRUE,
        sep= "," )
